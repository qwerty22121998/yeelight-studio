...
pub(crate) struct FlowBuf {
    buf: Vec<Rgb>,
    last: Instant,
    /// Last color actually flushed — the one the bulb now rests on (`FlowAction::Stay`).
    /// A window that stays within `dedup` of it animates nothing, so it's dropped.
    last_flushed: Option<Rgb>,
    dedup: u8,
}

impl FlowBuf {
    /// A fresh, empty buffer. `dedup` is the per-channel delta below which a whole window
    /// counts as unchanged and is skipped (should match the caller's instant-path dedup).
    pub(crate) fn new(dedup: u8) -> Self {
        Self { buf: Vec::new(), last: Instant::now(), last_flushed: None, dedup }
    }
    ...
    ///
    /// Also returns `None` (saving a command / quota) when the whole due window stays within
    /// `dedup` of the color the bulb already rests on — nothing to animate.
    pub(crate) fn push(&mut self, color: Rgb, gap: Duration, force: bool) -> Option<(Vec<Rgb>, u32)> {
        ...
        // Cache/dedup: if every color in the due window is within `dedup` of what the bulb
        // already shows, the flow would repaint the same thing — drop it, reset the window.
        if !force
            && let Some(prev) = self.last_flushed
            && self.buf.iter().all(|c| c.max_delta(prev) <= self.dedup)
        {
            self.buf.clear();
        ...
        self.last = Instant::now();
        self.last_flushed = colors.last().copied();
        Some((colors, step))
    }
}
...
mod tests {
    ...
    fn flow_buf_batches_a_window_then_flushes() {
        let mut fb = FlowBuf::new(DEDUP_DELTA);
    ...
    #[test]
    fn flow_buf_skips_unchanged_window() {
        let mut fb = FlowBuf::new(DEDUP_DELTA);
        let c = |n: u8| Rgb { r: n, g: n, b: n };
        // First forced flush establishes the resting color the bulb stays on.
        let (colors, _) = fb.push(c(100), Duration::ZERO, true).expect("first flush");
        assert_eq!(colors.last().copied(), Some(c(100)));
        // Windows within DEDUP_DELTA of that resting color animate nothing → skipped, no command.
        assert!(fb.push(c(102), Duration::ZERO, false).is_none(), "unchanged window skipped");
        assert!(fb.push(c(101), Duration::ZERO, false).is_none(), "still unchanged");
        // A real change flushes again.
        let (colors, _) = fb.push(c(200), Duration::ZERO, false).expect("changed window flushes");
        assert!(colors.contains(&c(200)));
    }
