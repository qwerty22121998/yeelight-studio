...
A warm, flat, monospace dark theme for `yeelight-gui`, modeled on
[claration/Impactor](https://github.com/claration/Impactor)'s "Plume Dark" look
(itself an iced app) — **but with the accent hue shifted from mauve-pink to
orange**.
...
## 1. What we're copying from Impactor

Impactor's identity is not one color — it's a *system*:
...
We keep the whole system and only re-hue the accent.

### Impactor reference palette ("Plume Dark")

| Role       | Hex        | Note                    |
|------------|------------|-------------------------|
| background | `#282021`  | warm near-black         |
| text       | `#f2d5cf`  | cream                   |
| primary    | `#d3869b`  | mauve-pink accent       |
| success    | `#d9a6b3`  | pink-family             |
| danger     | `#e78a8a`  | warm red                |
| warning    | `#f4b8c4`  | pink-family             |

---

## 2. Ember Dark palette (orange)

Same structure, orange accent, warm-brown base. Legible semantic colors are
kept distinct (see the note below) so "connected" never looks like "error".
...
> **Monochrome vs. legible.** Impactor makes success/danger/warning all
> pink-family for a cohesive look. That trades usability — a bulb-control UI
> needs "online" (green) to read differently from "error" (red) at a glance.
> Ember keeps the orange *accent* as the identity but keeps semantic states
> distinguishable. If you want the fully-monochrome Impactor feel, set
> success `#f0a35e`, danger `#e8735a`, warning `#f2b56a` instead.

### Surface elevation (derived, not authored)

Never author panel colors. Derive them from `background` the way Impactor does.
...
`lighten`/`darken` are Impactor's exact helpers — clamp each channel after
adding/subtracting a flat amount:
...
Adopt Impactor's constants verbatim (only the font is optional).
...
Ported from Impactor's `appearance/{button,picklist}.rs`, re-hued.
...
**Active chip / tab**: bg `primary`, text `background` (not white — matches
Impactor's text-on-accent rule).
...
1. **New module `src/theme.rs`** (or an `appearance/` dir mirroring Impactor):
...
2. **`app.rs`** — make Ember the default: `theme()` returns `ember_dark()`;
   ...
   the picker to match Impactor's single-theme model. Recommend keeping it —
   near-zero cost, Ember just becomes the default entry.
...
- **Font**: the whole app is monospace via `main.rs` `.default_font(iced::Font::MONOSPACE)`
  — the platform mono face (DejaVu/Menlo/Consolas), no bundled asset. To match
  Impactor's *exact* face, bundle `Hack-Regular.ttf` with `include_bytes!` +
  `.font(...)` and set `iced::Font::with_name("Hack")` as the default. That's the
  only remaining upgrade for pixel-fidelity.
